  ...
  useEffect(() => {
    const handleCredentialResponse = async (response) => {
      ...
      try {
        ...
        if (result.success) {
          ...
          setGlobalError(result.error || 'Failed to authenticate with Google.');
          setIsLoading(false);
        }
      } catch (err) {
        setGlobalError('Something went wrong during Google sign-in.');
        setIsLoading(false);
      }
    };
    ...
    const initGoogle = () => {
      if (typeof window !== 'undefined' && window.google) {
        ...
        const btn = document.getElementById("google-signup-button-overlay");
        if (btn) {
    ...
    <div className={styles.page}>
      ...
      <aside className={styles.brandPanel}>
        <div className={styles.brandPanelInner} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', width: '100%' }}>
          ...
          <p className={styles.brandTagline} style={{ textAlign: 'center', fontSize: '18px', color: '#6B6B6B', marginBottom: '40px', width: '100%', fontStyle: 'normal' }}>Begin preserving what matters most.</p>
          ...
          <div className={styles.card}>
            ...
            <div className={styles.socialButtons}>
              ...
              <div style={{ position: 'relative', width: '100%' }}>
                ...
                <div 
                  id="google-signup-button-overlay" 
                  style={{ 
                    position: 'absolute', 
                    top: 0, 
                    left: 0, 
